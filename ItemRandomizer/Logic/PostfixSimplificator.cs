...
namespace ItemRandomizer {
	class PostfixSimplificator {
		...
		public PostfixSimplificator(IEnumerable<string> postfix) : this(string.Join(" ", postfix.ToArray())) { }

		public PostfixSimplificator(string postfix) {
			_postfix = postfix;
		...
		private class PFSNode : IComparable<PFSNode> {
			private const string AND_PLUS = "+";
			private const string AND_PERSAND = "&";
			private const string AND = AND_PERSAND;
			...
			public void Print(string depth = "") {
				...
				_PrintLine(string.Join(OpType == OpType.OR ? $" {OR} " : $" {AND} ", thing.ToArray()));
			...
			OpType _OpTypeOf(string token) {
				if (token == OR)
					return OpType.OR;
				if (token == AND_PLUS || token == AND_PERSAND)
					return OpType.AND;
			...
			private List<string> _Prefix() {
				...
				foreach (PFSNode node in Nodes) {
					...
					prefix.Add(this.OpType == OpType.OR ? OR : AND);
					if (node.OpType == OpType.OPERAND) {
